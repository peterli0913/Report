...
|---|---|---|
| 时间趋势 | `line` / `bar` | 饼图 |
| 趋势 vs 目标 | `line(target=95)` 带参考线 | 只在标题里写目标值 |
...
- **柱状图画不了目标参考线**（PowerPoint 组合图 python-pptx 不支持）。改用
  `progress_bars`，或把目标写进结论条。
- **`progress_bars` 在值域集中（如 94-98）时条长差不到 8%**，"谁没达标"只能靠颜色和
  参考线读。这种情况改用表格把数值和缺口直接列出来更清楚。
